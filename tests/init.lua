...
local add = eval.dofile("tests/add.lsp")
assert(add == 65, "Addition failed! ".. add)

local mathy = eval.dofile("tests/math.lsp")
assert(mathy == -40, "Math failed! " .. mathy)

local listy = eval.dofile("tests/list.lsp")
...
local fac, fib = unpack(eval.dofile("tests/y.lsp"))
